...
# Add Customer is available only on the Customers page. New customers always start Active.
s, n = re.subn(
    r"function addCustomer\(\)\{openSheet\(`<h3>Add Customer</h3>(.*?)<select id=\\\"st\\\" class=\\\"select\\\">.*?</select><button class=\\\"btn green full\\\" onclick=\\\"saveCustomer\(\)\\\">Save Customer</button>`\)\}",
    r'''function addCustomer(){openSheet(`<h3>Add Customer</h3><div class="muted" style="margin-bottom:6px">New customers are automatically Active. Status will update automatically from expiry and bill payment.</div>\1<div class="detail"><span>Initial Status</span><b class="success">ACTIVE</b></div><button class="btn green full" onclick="saveCustomer()">Save Customer</button>`)}''',
    s,
    count=1,
    flags=re.S,
)
...
# If the payment-flow patch has added Client Code, keep it while forcing initial status to Active.
if "status:st.value,createdAt:today()" in s:
    s = s.replace("status:st.value,createdAt:today()", "status:'active',createdAt:today()", 1)
elif "status:'active',createdAt:today()" not in s:
    raise SystemExit('Could not find customer status field')

# A fully paid expired customer becomes Active again and gets the current month's end as expiry.
old = "function savePayment(id){let amount=Number(amt.value||0);if(amount<=0)return;"
if old not in s:
    raise SystemExit('Could not find savePayment')
# Replace the complete function only once, regardless of whether Client Code patches have changed nearby source.
m = re.search(r"function savePayment\(id\)\{.*?\}\n?function details", s)
if not m:
