...
# Any positive payment on an expired customer immediately reactivates them.
# Example: 500 due + 1 paid => Active, Paid 1, Due 499.
m = re.search(r'function savePayment\(id\)\{.*?\}\s*function details', s)
if m:
    new_func = "function savePayment(id){let amount=Number(amt.value||0);if(amount<=0)return;let c=d.customers.find(x=>String(x.id)===String(id));if(!c)return;let wasExpired=c.status==='expired';let beforeDue=Math.max(0,Number(due(c)||0));d.payments.push({id:Date.now().toString(),customer:id,amount,date:today(),month:month(),status:'paid',source:'manual',ref:ref.value||''});let remaining=Math.max(0,beforeDue-amount);let prev=Number(c.prevDue||0);if(prev>0){let applied=Math.min(prev,amount);c.prevDue=Math.max(0,prev-applied);}if(amount>0){c.status='active';let x=new Date();x.setMonth(x.getMonth()+1,0);c.expiry=x.getFullYear()+'-'+pad(x.getMonth()+1)+'-'+pad(x.getDate());}save();closeSheet();toast(wasExpired?'Payment Saved — Customer Active — Due '+remaining:(remaining>0?'Partial Payment Saved — Due '+remaining:'Bill Paid — Customer Active'));render()}function details"
    s = s[:m.start()] + new_func + s[m.end():]

p.write_text(s, encoding='utf-8')
print('Expired partial-payment reactivation rule patched successfully.')
