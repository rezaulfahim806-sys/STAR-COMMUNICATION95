...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'STAR COMMUNICATION bKash API',sandbox:BKASH_BASE_URL.includes('sandbox')}));

app.post('/api/bkash/create',async(req,res)=>{
  try{
    const {customerCode,customerName,amount,merchantInvoiceNumber}=req.body||{};
    ...
    const callback=`${PUBLIC_URL}/api/bkash/callback?code=${encodeURIComponent(customerCode)}&name=${encodeURIComponent(customerName||'Customer')}&amount=${encodeURIComponent(value)}`;
...
app.get('/api/bkash/callback',async(req,res)=>{
  ...
  try{
    ...
    const token=await getToken();
    const data=await bkashFetch('/checkout/payment/execute',{method:'POST',headers:{authorization:token,'x-app-key':BKASH_APP_KEY},body:JSON.stringify({paymentID})});
    const code=String(req.query.code||'');
    const name=String(req.query.name||'Customer');
    const amount=String(req.query.amount||data.amount||'');
    const url=`${PAYMENT_PAGE_URL}?status=success&paymentID=${encodeURIComponent(paymentID)}&trxID=${encodeURIComponent(data.trxID||'')}&code=${encodeURIComponent(code)}&name=${encodeURIComponent(name)}&amount=${encodeURIComponent(amount)}`;
    res.redirect(url);
  }catch(e){res.redirect(`${PAYMENT_PAGE_URL}?status=error&paymentID=${encodeURIComponent(paymentID)}`)}
});

app.post('/api/bkash/execute',async(req,res)=>{
  try{const paymentID=String(req.body?.paymentID||'');if(!paymentID)return res.status(400).json({error:'paymentID is required'});const token=await getToken();const data=await bkashFetch('/checkout/payment/execute',{method:'POST',headers:{authorization:token,'x-app-key':BKASH_APP_KEY},body:JSON.stringify({paymentID})});res.json(data)}catch(e){res.status(502).json({error:e.message||'bKash execute failed'})}
});
